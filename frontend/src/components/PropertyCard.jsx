...
export default function PropertyCard({ property, showActions = false }) {
  ...
  return (
    <>
      ...
      {/* Property Card */}
      <div className="bg-card rounded-xl shadow-card p-4 max-w-sm w-full h-[580px] flex flex-col mx-auto transition-all duration-300 hover:shadow-xl overflow-hidden"> {/* Added overflow-hidden */}
        {/* Image Carousel */}
        <div
          className="relative w-full flex flex-col items-center"
          onTouchStart={handleTouchStart}
        ...
        {/* Property Details */}
        <div className="mt-4 flex-1 overflow-hidden"> {/* Added overflow-hidden */}
          <h4 className="text-lg font-bold text-primary truncate font-sans">{property.title}</h4>
          <p className="text-neutral text-sm mt-1 line-clamp-2 font-sans">{property.description}</p>
          <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-neutral font-sans">
            {property.location && (
          ...
          {showActions ? (
            <>
              ...
              >
                <FiShare2 className="h-5 w-5" />
                {copied && (
                  <span className="absolute -top-7 left-1/2 -translate-x-1/2 bg-primary text-white text-xs rounded-md px-2 py-1 shadow-sm font-sans z-10"> {/* Added z-10 */}
                    Link Copied!
                  </span>
                )}
              </button>
