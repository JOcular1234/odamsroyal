  ...
  return (
    <header className="w-full bg-gray-900 text-white shadow flex items-center justify-between px-4 sm:px-8 py-4">
      ...
      <nav className="hidden md:flex gap-6 items-center">
        ...
        <form method="POST" action="/api/admin/logout" className="inline-block ml-4">
          <button
            type="submit"
            className="flex items-center gap-1 bg-[#f97316] text-white px-4 py-2 rounded font-semibold hover:bg-[#e86a15] transition-colors"
          >
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 bg-black bg-opacity-50 md:hidden" onClick={() => setMenuOpen(false)}>
          ...
          >
            ...
            <form method="POST" action="/api/admin/logout" className="mt-4">
              <button
                type="submit"
                className="flex items-center gap-2 w-full bg-[#f97316] text-white px-4 py-2 rounded font-semibold hover:bg-[#e86a15] transition-colors"
              >
                <ArrowRightOnRectangleIcon className="w-5 h-5 mr-1 inline-block" />
                Logout
              </button>
