...
export default function Home() {
  ...
  const PAGE_SIZE = 10;
  const CARD_HEIGHT = 580; // Match PropertyCard height exactly
  const CARD_WIDTH = 380; // Match PropertyCard width
  const CARD_PADDING = 20; // Account for padding between cards
  ...
  // Calculate proper grid height based on actual column count
  const getGridHeight = useCallback(() => {
    const columnCount = window.innerWidth >= 1024 ? 3 : window.innerWidth >= 640 ? 2 : 1;
    const rowCount = Math.ceil(filteredProperties.length / columnCount);
    return Math.max(rowCount * CARD_HEIGHT, 600);
      ...
      <main>
        ...
        >
          ...
          {loading && properties.length === 0 ? (
            ...
            <ErrorBoundary>
              <div style={{ height: getGridHeight(), overflowY: 'auto', overflowX: 'hidden' }}>
                <AutoSizer>
                  {({ height, width }: { height: number; width: number }) => {
                    const columnCount = width >= 1024 ? 3 : width >= 640 ? 2 : 1;
                    const rowCount = Math.ceil(filteredProperties.length / columnCount);
                    
                    // Calculate available width per column to prevent horizontal scroll
                    const availableWidth = width - 20; // Account for container padding
                    const columnWidth = Math.min(CARD_WIDTH + CARD_PADDING, availableWidth / columnCount);
                    ...
                    return (
                      <FixedSizeGrid
                        ...
                        height={height}
                        rowCount={rowCount}
                        rowHeight={CARD_HEIGHT + CARD_PADDING}
                        width={width}
