...
//   const id = params?.id; // Alternative typing to avoid TS error
...
//     console.log('ID:', id); // Debug: Check if id is defined
//     window.scrollTo({ top: 0, left: 0, behavior: 'auto' });
//     async function fetchProperty() {
//       // Fetch Other Properties in parallel
//       fetchOtherProperties();
//       if (!id) {
//         console.log('No ID provided'); // Debug: Log missing ID
...
//       } catch (_error: any) {
//         console.error('Fetch error:', _error.message, _error.response?.data); // Debug: Log error details
...
//     } catch (_error: any) {
//       console.error('Inquiry error:', _error.message); // Debug: Log inquiry error
//       setFormError('Failed to send inquiry.');
//     } finally {
//       setSubmitting(false);
//     }
//   };
...
//   // Fetch all properties and pick random others (excluding current)
//   async function fetchOtherProperties() {
//     setOtherLoading(true);
//     setOtherError(null);
//     try {
//       const res = await axios.get<Property[]>(`${API_URL}/properties`);
//       let others = res.data.filter((p) => p._id !== id);
//       // Shuffle array
//       for (let i = others.length - 1; i > 0; i--) {
//         const j = Math.floor(Math.random() * (i + 1));
//         [others[i], others[j]] = [others[j], others[i]];
//       }
//       setOtherProperties(others.slice(0, 6));
//     } catch (err: any) {
//       setOtherError('Failed to load other properties');
//       setOtherProperties([]);
//     } finally {
...
//                         className={`w-28 h-20 rounded-lg object-cover border cursor-pointer shadow-sm ${
...
//         <h2 className="text-2xl md:text-3xl font-bold text-[#f97316] mb-6 font-sans text-center">Other Properties</h2>
//         {otherLoading ? (
  ...
  useEffect(() => {
    ...
    async function fetchProperty() {
      ...
      try {
        ...
        console.error('Fetch error:', _error.message, _error.response?.data);
        setError('Failed to fetch property details');
        setProperty(null);
      } finally {
        setLoading(false);
      ...
      >
        {error ? (
          ...
          <>
            ...
            {/* Footer */}
            <footer className="mt-12 pt-6 border-t border-[#f97316]/20 text-center">
              <img
                src="https://res.cloudinary.com/drkli5pwj/image/upload/v1753880711/odamzlogo_cqgm0c.jpg"
                alt="Odamz Royal Consultz Logo"
                className="mx-auto mb-4 max-w-32"
              />
              <p className="text-gray-600 font-sans">
                📍 CBD, Abuja (Center Business District), Nigeria<br />
                📞 <a href="tel:+2347061198858" className="text-[#f97316] hover:underline">07061198858</a>,
                <a href="tel:+2348123485718" className="text-[#f97316] hover:underline">08123485718</a><br />
                📧 <a href="mailto:info.odamzroyalty@gmail.com" className="text-[#f97316] hover:underline">
